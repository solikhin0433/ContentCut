...
async function getVideoInfo() {
    ...
    try {
        ...
        document.getElementById('endTime').value = secToTime(Math.min(d.duration, 60));

        show('infoCard'); show('step-cut');
    } catch(e) { showErr(e.message); }
    finally    { btn.disabled = false; btn.textContent = 'Cek Video'; }
}
...
async function cutVideo() {
    ...
    hide('step-cut'); show('progressCard'); hideErr(); startProgress();

    try {
        const res = await fetch('/api/video/cut', {
            method: 'POST', headers: {'Content-Type':'application/json'},
            body: JSON.stringify({ url, start_time: start, end_time: end, quality: qual })
        });

        if (!res.ok) { const d = await res.json(); throw new Error(d.detail || 'Gagal proses'); }
        ...
        stopProgress(); show('step-cut');
        alert('✅ Video berhasil dipotong! File sudah terdownload.');
    } catch(e) {
        stopProgress(); show('step-cut'); showErr(e.message);
    }
}
...
function startProgress() {
    ...
    progressTimer = setInterval(() => {
        if (step < STEPS.length) {
            ...
            if (progress < target) { progress += 1.5; fill.style.width = progress + '%'; }
            else { document.getElementById('progressMsg').textContent = msg; step++; }
        }
    }, 150);
}
